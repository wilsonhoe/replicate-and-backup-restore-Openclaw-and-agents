...
// Usage: node post-dayX-zoho-fixed.js 3 (for Day 3)
...
function loadContent(filePath) {
  ...
  // Extract Twitter version - updated to match actual format
  const twitterMatch = content.match(/## Twitter Version \(Under 250 chars\)\n\n([\s\S]+?)(?=\n## |\n---|$)/);
  ...
  // Fallback: if Twitter not found with exact match, try flexible matching
  if (!twitterContent) {
    const twitterMatch2 = content.match(/## Twitter Version\s*\n\n([\s\S]+?)(?=\n## |\n---|$)/);
    const twitterContent2 = twitterMatch2 ? twitterMatch2[1].trim() : '';
    if (twitterContent2) {
      console.log('⚠️ Using flexible Twitter match');
    }
    return { twitter: twitterContent2, linkedin: linkedinContent };
  }
...
(async () => {
  ...
  try {
    ...
    if (content.twitter.length === 0) {
      console.warn('⚠️ Twitter content is empty - checking file format...');
    }
  } catch (err) {
