// Fixed Cookie Import Playwright Script
...
// Function to extract Twitter content (part after "## Twitter Version")
function extractTwitterContent(fullContent) {
  const parts = fullContent.split('---\\n');
  // Part 1 contains "## Twitter Version" header + content
  const twitterSection = parts[1];
  // Extract content after the header line
  const lines = twitterSection.split('\\n');
...
// Function to extract LinkedIn content (part after "## LinkedIn Version")
function extractLinkedInContent(fullContent) {
  const parts = fullContent.split('---\\n');
  // Part 2 contains "## LinkedIn Version" header + content
  const linkedinSection = parts[2];
  // Extract content after the header line
...
(async () => {
  ...
  try {
    ...
    // Type content - skip characters that can't be keyboard pressed
    const typeableChars = twitterContent.split('').filter(c => {
      // Only include standard ASCII typeable chars
      const code = c.charCodeAt(0);
      return code >= 32 && code <= 126;
    }).join('');

    for (const char of typeableChars) {
      await twitterPage.keyboard.press(char === '\n' ? 'Enter' : char);
      await twitterPage.waitForTimeout(10);
    }
    await twitterPage.waitForTimeout(500);
