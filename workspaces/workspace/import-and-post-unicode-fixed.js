// Unicode-fixed Cookie Import Playwright Script
...
function extractTwitterContent(fullContent) {
  ...
  // Split by any combination of line endings
...
// Function to check if a character is safe for keyboard.press
function isSafeForKeyboardPress(char) {
...
(async () => {
  ...
  try {
    ...
    // Method 1: Try typing character by character for safe ASCII, fallback to fill for Unicode
    ...
    // Check if content contains unsafe Unicode characters
    const hasUnsafeChars = [...twitterContent].some(char => !isSafeForKeyboardPress(char));
    
    if (!hasUnsafeChars) {
      // Safe to type character by character
      console.log('Typing content character by character...');
      for (let i = 0; i < twitterContent.length; i++) {
        const char = twitterContent[i];
        if (char === '\n') {
          await twitterPage.keyboard.press('Enter');
        } else if (char === '\t') {
          await twitterPage.keyboard.press('Tab');
        } else {
          await twitterPage.keyboard.press(char);
        }
        await twitterPage.waitForTimeout(12);
      }
    } else {
      // Contains Unicode, use fill method
      console.log('Content contains Unicode characters, using fill method...');
      await textarea.fill(twitterContent);
