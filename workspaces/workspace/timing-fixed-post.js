...
(async () => {
  ...
  try {
    console.log('Starting timing-fixed cookie-based posting...');
    ...
    await twitterPage.waitForTimeout(3000); // Increased initial wait
    console.log('Current URL:', await twitterPage.url());
    
    // Wait for the editor to be ready with longer timeout and retry logic
    ...
    while (!editorReady && attempts < maxAttempts) {
      try {
        ...
        // Double-check it's actually interactable
        ...
        if (isVisible && isEnabled) {
          ...
          console.log(`Attempt ${attempts + 1}: Composer found but not interactable (visible: ${isVisible}, enabled: ${isEnabled})`);
        }
      } catch (error) {
      ...
      if (!editorReady && attempts < maxAttempts) {
        await twitterPage.waitForTimeout(2000); // Wait before retry
      }
    }
    ...
    // Additional verification: check if content was entered correctly
    const enteredContent = await twitterPage.evaluate(() => {
      const elem = document.querySelector('[contenteditable="true"]');
      return elem ? elem.innerText : '';
    });
    console.log('Entered content length:', enteredContent.length);
    console.log('Expected content length:', twitterContent.length);
